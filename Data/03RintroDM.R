library(tidyverse) #open tidyverse library
search()
#############
...
link = "D:\\CAWU 5\\DATA MINING\\R\\Data Mining\\Data\\CrimeUS.xlsx"
fd <- "D:\\CAWU 5\\DATA MINING\\R\\Data Mining\\Data\\CrimeUS.xlsx"
dat <- read_excel(fd,sheet=1)
class(dat)
dat

datZ=read.table("clipboard",header=TRUE,na="???")

datY=as.tibble(read.table("clipboard",header=TRUE,na.strings = "???"))
...
std2 <- read_delim("stud2.txt", delim=" ",
                   skip=0, col_names=c("StudentName","Math","English","Degree_Year"))
std2
...
# ux[which((cummax(tabulate(match(algae$mxPH, ux)))-tabulate(match(algae$mxPH, ux)))==0)]
}
